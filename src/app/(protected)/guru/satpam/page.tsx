...
import { ArrowLeft, PlusCircle, Loader2, AlertCircle, Pencil, Trash2, ShieldQuestion, Calendar as CalendarIcon } from "lucide-react";
...
const ACTIVITY_ID = 'satpam';
const ACTIVITY_NAME = 'Satpam';
  ...
  const onSubmit = async (data: LaporanFormData) => {
      ...
      try {
          if (reportToModify) { // Editing
              ...
              await addActivityLog("Laporan Satpam Diperbarui", `Laporan "${data.title}" diperbarui oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Satpam');
              ...
              await addLaporanKegiatan({
                  ...
                  createdByUid: userProfile.uid, createdByDisplayName: userProfile.displayName || 'Satpam',
              });
              await addActivityLog("Laporan Satpam Dibuat", `Laporan "${data.title}" dibuat oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Satpam');
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground font-headline flex items-center gap-2">
            <ShieldQuestion className="h-8 w-8 text-primary" /> Dasbor Satpam
          </h1>
          <p className="text-muted-foreground">Catat dan kelola laporan kegiatan keamanan.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Laporan Kegiatan Satpam</CardTitle>
          ...
          <DialogContent className="sm:max-w-[525px]">
              <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)}>
                      <DialogHeader><DialogTitle>{reportToModify ? 'Edit' : 'Buat'} Laporan Satpam</DialogTitle><DialogDescription>Isi detail laporan kegiatan keamanan.</DialogDescription></DialogHeader>
                      <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-4">
                          <FormField control={form.control} name="title" render={({ field }) => (<FormItem><FormLabel>Judul Laporan</FormLabel><FormControl><Input placeholder="cth: Laporan Keamanan Shift Malam" {...field} /></FormControl><FormMessage /></FormItem>)} />
