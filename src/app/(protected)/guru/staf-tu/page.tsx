...
import { ArrowLeft, PlusCircle, Loader2, AlertCircle, Pencil, Trash2, Users as UsersIcon, Calendar as CalendarIcon } from "lucide-react";
...
const ACTIVITY_ID = 'staf_tu';
const ACTIVITY_NAME = 'Staf Tata Usaha';

export default function StafTuDashboardPage() {
  ...
  const onSubmit = async (data: LaporanFormData) => {
      ...
      try {
          if (reportToModify) { // Editing
              ...
              await addActivityLog("Laporan Staf TU Diperbarui", `Laporan "${data.title}" diperbarui oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Staf TU');
              ...
              await addLaporanKegiatan({
                  ...
                  createdByUid: userProfile.uid, createdByDisplayName: userProfile.displayName || 'Staf TU',
              });
              await addActivityLog("Laporan Staf TU Dibuat", `Laporan "${data.title}" dibuat oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Staf TU');
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground font-headline flex items-center gap-2">
            <UsersIcon className="h-8 w-8 text-primary" /> Dasbor Staf Tata Usaha
          </h1>
          <p className="text-muted-foreground">Catat dan kelola laporan kegiatan Anda sebagai Staf TU.</p>
        </div>
      </div>
          ...
          <DialogContent className="sm:max-w-[525px]">
              <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)}>
                      <DialogHeader><DialogTitle>{reportToModify ? 'Edit' : 'Buat'} Laporan</DialogTitle><DialogDescription>Isi detail laporan kegiatan Staf Tata Usaha.</DialogDescription></DialogHeader>
                      <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-4">
                          <FormField control={form.control} name="title" render={({ field }) => (<FormItem><FormLabel>Judul Laporan</FormLabel><FormControl><Input placeholder="cth: Pemberkasan Dokumen Siswa Baru" {...field} /></FormControl><FormMessage /></FormItem>)} />
