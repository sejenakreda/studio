...
import { getStudentById, updateStudent } from '@/lib/firestoreService';
...
const editStudentSchema = z.object({
  ...
  // NIS and id_siswa are not part of the schema as they are read-only
});
...
export default function EditStudentPage() {
  ...
  const studentId = params.studentId as string; // Firestore document ID
  ...
  const fetchStudentData = useCallback(async (id: string) => {
    if (!id) {
      ...
      setFetchError("ID Siswa tidak valid.");
      toast({ variant: "destructive", title: "Error", description: "ID Siswa tidak ditemukan." });
      router.push('/guru/students'); // Redirect if no ID
      return;
    }
  ...
  const onSubmit = async (data: EditStudentFormData) => {
    if (!studentId || !studentData) {
      toast({ variant: "destructive", title: "Error", description: "Data siswa tidak lengkap untuk pembaruan." });
      return;
    }
  ...
  if (fetchError || !studentData) {
    return (
      <div className="space-y-6">
        ...
        <Button onClick={() => router.push('/guru/students')}>Kembali ke Daftar Siswa</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground font-headline">Edit Data Siswa</h1>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardHeader>
              ...
              <CardDescription>Ubah nama atau kelas siswa. NIS dan ID Siswa tidak dapat diubah.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
