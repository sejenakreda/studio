...
import { ArrowLeft, PlusCircle, Loader2, AlertCircle, Pencil, Trash2, Briefcase, Calendar as CalendarIcon } from "lucide-react";
...
const ACTIVITY_ID = 'kepala_tata_usaha';
const ACTIVITY_NAME = 'Tata Usaha';
  ...
  const onSubmit = async (data: LaporanFormData) => {
      ...
      try {
          if (reportToModify) { // Editing
              ...
              await addActivityLog("Laporan Tata Usaha Diperbarui", `Laporan "${data.title}" diperbarui oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Kepala TU');
              ...
              await addLaporanKegiatan({
                  ...
                  createdByUid: userProfile.uid, createdByDisplayName: userProfile.displayName || 'Kepala TU',
              });
              await addActivityLog("Laporan Tata Usaha Dibuat", `Laporan "${data.title}" dibuat oleh ${userProfile.displayName}`, userProfile.uid, userProfile.displayName || 'Kepala TU');
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground font-headline flex items-center gap-2">
            <Briefcase className="h-8 w-8 text-primary" /> Dasbor Tata Usaha
          </h1>
          <p className="text-muted-foreground">Catat dan kelola laporan kegiatan administrasi.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Laporan Kegiatan Tata Usaha</CardTitle>
          ...
          <DialogContent className="sm:max-w-[525px]">
              <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)}>
                      <DialogHeader><DialogTitle>{reportToModify ? 'Edit' : 'Buat'} Laporan Tata Usaha</DialogTitle><DialogDescription>Isi detail laporan kegiatan administrasi.</DialogDescription></DialogHeader>
                      <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-4">
                          <FormField control={form.control} name="title" render={({ field }) => (<FormItem><FormLabel>Judul Laporan</FormLabel><FormControl><Input placeholder="cth: Pengarsipan Surat Masuk Bulan Ini" {...field} /></FormControl><FormMessage /></FormItem>)} />
