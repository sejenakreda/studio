...
import { PlusCircle, Loader2, AlertCircle, Trash2, Edit, FileSignature, Image as ImageIcon } from "lucide-react";
...
export default function BeritaAcaraPage() {
    ...
    const fetchRiwayat = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await getBeritaAcara();
    ...
    }, [fetchRiwayat, userProfile, editingBeritaAcara, form]);
    ...
    const onSubmit = async (data: BeritaAcaraFormData) => {
        ...
        try {
            if (editingBeritaAcara) {
                ...
                await addBeritaAcara({ ...data, createdByUid: userProfile.uid, createdByDisplayName: userProfile.displayName || "Pengguna" });
    ...
    const handleDelete = async () => {
        if (!beritaAcaraToDelete) return;
        try {
            await deleteBeritaAcara(beritaAcaraToDelete.id!);
    ...
    const handlePrint = (id: string) => {
        router.push(`/protected/administrasi-ujian/print/${id}`);
    };
